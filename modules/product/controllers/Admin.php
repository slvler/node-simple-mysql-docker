...
class Admin extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->load->model('product/Admin_model');
	...
	public function index()
	{
		...
		if ($this->migration->init_module("product")) { $this->migration->current(); }
		...
		$this->load->view('product/admin/admin', $this->data);
	}
	...
	public function change_active()
	{
		...
		foreach(get_lang_id_records($id, "product") as $item){
		...
		if($_POST){
			...
			foreach(all_languages() as $item){
				if($this->input->post()[$item->lang]["title"] != NULL){			
					$add_id = $this->Admin_model->add_record($id, $this->input->post()[$item->lang], $item->lang, md5(date('YmdHis')));
			...
			redirect('/product/admin/index/'.$id, 'refresh');
		}else{
			$this->load->view('product/admin/add_record');
		}
	}
	...
	public function delete_record()
	{
		...
		// Seçilen kaydın farklı dillerini bulup aynı işlemi onlar için de uyguluyor
		foreach(get_lang_id_records($id, "product") as $item){
			// product tablosundaki asıl veriyi siliyor
			$this->Admin_model->delete_record($item->id);
			// Routes tablosundaki linki siliyor
			delete_seo_url("product/index/".$item->id);
		...
		redirect('/product/admin/index/'.$find_parent['parent'], 'refresh');
	}
	...
	public function edit_record()
	{
		if($_POST){		
			...
			redirect('product/admin/edit_record/'.$id, 'refresh');
		}else{
			$id = (int)($this->uri->segment(4));
			...
			$this->load->view('product/admin/edit_record', $this->data);
		}
	}
	...
	public function update_list()
	{
		...
		foreach ($_GET['listItem'] as $item2){
			// Seçilen kaydın farklı dillerini bulup aynı işlemi onlar için de uyguluyor
			foreach(get_lang_id_records($item2, "product") as $item){
	...
	public function add_gallery_image($parent, $lang)
	{
	    ...
	    for($i=0; $i<$cpt; $i++){
			...
			$this->Admin_model->add_gallery_image($parent, img_upload($lang.'_gallery', "product_gallery"));
	    }
